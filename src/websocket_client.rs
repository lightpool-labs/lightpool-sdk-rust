...
impl WebSocketClient {
    ...
    pub async fn subscribe(
        ...
        tokio::spawn(async move {
            Self::handle_messages(ws_stream_clone, sender_clone, Some(id_sender)).await;
        });

        drop(ws_stream_guard);

        tokio::time::timeout(std::time::Duration::from_secs(10), id_receiver)
            .await
            .map_err(|_| {
                SdkError::Timeout("Timed out waiting for subscription confirmation".to_string())
            })?
                                ...
                                if let Some(params) = value.get("params") {
                                    if let Some(result) = params.get("result") {
                                        if let Some(subscription_id) =
                                            result.get("subscription_id").and_then(|s| s.as_str())
                                        {
